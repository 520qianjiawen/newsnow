...
export function getCardTheme(color: Color, index: number): CardVars {
  ...
  return {
    ...
    "--card-base-alpha": `${0.18 + emphasis}`,
    "--card-border-alpha": `${0.22 + emphasis / 2}`,
    "--card-shine-alpha": `${0.16 + emphasis / 2}`,
    "--card-bloom-alpha": `${0.24 + emphasis / 2}`,
    "--card-wash-alpha": `${0.34 + emphasis}`,
    "--card-panel-top": `${0.56 - Math.min(emphasis, 0.08) / 2}`,
    "--card-panel-bottom": `${0.82 - Math.min(emphasis, 0.08) / 2}`,
  }
}
